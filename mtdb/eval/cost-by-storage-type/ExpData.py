...
def Load():
	with Cons.MeasureTime("Loading exp data ..."):
		_LoadExpGroup("cold-ebs-mag")
		_LoadExpGroup("cold-ebs-ssd")


def _LoadExpGroup(expg):
	exps = []
	for exp, exp_datetime in Conf.Get(expg).iteritems():
	...
	# TODO: save to file and plot
	fmt = "%5s %10.6f %10.6f %10.6f"
	Cons.P(Util.BuildHeader(fmt, "exp_name hot_stg_cost cold_stg_cost total_stg_cost"))
	for e in exps:
		Cons.P(fmt % (e.exp_name, e.hot_stg_cost, e.cold_stg_cost, e.hot_stg_cost + e.cold_stg_cost))
