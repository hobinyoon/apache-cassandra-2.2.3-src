...
# TODO: all 3 in one func
test_4k_rand_read() {
	FN_TEST=${DN_LOCAL_SSD}/ioping-test

	dn_test=$1

	FN_TEST=${dn_test}/ioping-test
	FN_OUT=${DN_OUTPUT}/4k-rand-read-${dn_test}-${DATETIME}

	# To make sure ram disk doesn't get full. Don't need for now.
	# \rm ${DN_OUTPUT}/*

	#vmtouch -q -t ${FN_TEST}

	# We are interested in direct IO, not cached IO. I see like 100% iowait.
	time ioping -D -c 100000 -i 0 ${FN_TEST} > ${FN_OUT}

	mkdir -p data || true
	mv ${FN_OUT} data/
	FN_OUT2=data/4k-rand-read-cached-local-ssd-${DATETIME}

	tail -n 2 $FN_OUT2
	printf "  Created %s %d\n" $FN_OUT2 `wc -c < $FN_OUT2`

	# This can be simplified
	#   4.0 KiB from /mnt/local-ssd/ioping-test (ext3 /dev/xvdb): request=999983 time=1 us
	# to. Leave the unit at the end. It can be us, ms, or anything.
	#   1 us
	# TODO: use awk or something. may want python.

	printf "  Manually edit the file and reduce size\n"

	# TODO: monitor iowait time during the experiment
	#   use sar or something.
}
...
test_4k_rand_read_direct_local_ssd() {
	FN_OUT=${DN_OUTPUT}/4k-rand-read-direct-local-ssd-${DATETIME}
	echo $FN_OUT
	time ioping -Dq -c 4000 -i 0 $DN_LOCAL_SSD/ioping-test | tee $FN_OUT
	printf "Created %s %d\n" $FN_OUT `wc -c < $FN_OUT`
}
...
gen_test_files() {
	...
	echo "Creating test files ..."
	time dd bs=1024 count=262144 < /dev/urandom > $DN_LOCAL_SSD/ioping-test
	time cp $DN_LOCAL_SSD/ioping-test $DN_NFS_HDD/ioping-test
}
...
vmtouch_test_file() {
	...
	time vmtouch -t $DN_LOCAL_SSD/ioping-test
...
test_4k_rand_read_cached_local_hdd() {
	...
	time ioping -Cq -c 10000000 -i 0 $DN_LOCAL_SSD/ioping-test | tee $FN_OUT
	printf "Created %s %d\n" $FN_OUT `wc -c < $FN_OUT`
}
...
test_4k_rand_read_direct_local_hdd() {
	...
	echo $FN_OUT
	time ioping -Dq -c 20000 -i 0 $DN_LOCAL_SSD/ioping-test | tee $FN_OUT
	printf "Created %s %d\n" $FN_OUT `wc -c < $FN_OUT`
}
...
test_256m_seq_write_local_hdd() {
	for ((i=0; i<10; i++)); do time dd bs=256M count=1 if=/dev/zero of=$DN_LOCAL_SSD/dd-test; done
}
...
test_256m_seq_write_direct_local_hdd() {
	for ((i=0; i<10; i++)); do time dd bs=256M count=1 oflag=direct if=/dev/zero of=$DN_LOCAL_SSD/dd-test; done
}
